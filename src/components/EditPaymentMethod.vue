<script setup>
	...
	const { paymentInfo, orderAmounts } = storeToRefs(orderStore)
...
<template>
	<v-card class="pb-2 pl-2 pr-2 pt-4">
		...
		<v-radio-group
			...
			true-icon="ic-outline-radio-button-checked"
			false-icon="ic-outline-radio-button-unchecked"
		>
			<v-container>
				<v-radio class="" value="card">
					<template v-slot:label
						><span class="text-caption">Credit or Debit Card</span>
					</template>
				</v-radio>
			...
			<v-container>
				<v-radio class="" value="cash_on_delivery">
					<template v-slot:label
						><span class="text-caption">Cash on Delivery</span>
					</template>
				</v-radio>
			</v-container>
