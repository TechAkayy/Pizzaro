    ...
    <v-radio-group class="" v-model="paymentInfo.method" hide-details>
      <v-container>
        <v-radio class="" value="card">
          <template v-slot:label><span class="font-weight-bold text-secondary-darken-1 text-subtitle-2">Credit or Debit
              Card</span>
          </template>
        </v-radio>
        <v-row class="" v-if="paymentInfo.method === 'card'">
          <v-col>
            <v-text-field density="compact" variant="outlined" placeholder="card number"
              v-model="cardDetails.cardNumber" :rules="cardNumberRules"></v-text-field>
          </v-col>
        </v-row>
        <v-row v-if="paymentInfo.method === 'card'">
          <v-col>
            <v-text-field variant="outlined" density="compact" placeholder="mm/yy" v-model="cardDetails.expiry"
              :rules="cardExpiryRules"></v-text-field>
          </v-col>
          <v-col cols="4">
            <v-text-field density="compact" variant="outlined" placeholder="cvv" v-model="cardDetails.cvv"
      ...
      <v-container>
        <v-radio class="" value="cash_on_delivery">
          <template v-slot:label><span class="font-weight-bold text-secondary-darken-1 text-subtitle-2">Cash on
              Delivery</span>
          </template>
        </v-radio>
      </v-container>
